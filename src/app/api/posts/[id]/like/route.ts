...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = await prisma.interaction.findUnique({
    where: { userId_postId_type: { userId: session.user.id, postId: params.id, type: "like" } },
  });

  const post = await prisma.post.findUnique({ where: { id: params.id }, select: { authorId: true, upvotes: true } });
  ...
  if (existing) {
    ...
    await prisma.interaction.create({
      data: { userId: session.user.id, postId: params.id, type: "like" },
    });
    const updated = await prisma.post.update({
      where: { id: params.id },
